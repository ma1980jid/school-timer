(function () {
  ...
  async function loadSettings() {
    ...
    try {
      const result = await c
        ...
        .select(
          "enabled,before_end_enabled,before_end_minutes,end_enabled,phone_notification_enabled,updated_at",
        )
        .eq("school_slug", slug)
        .maybeSingle();
      if (!result.error && result.data) {
        settings = {
          ...
          beforeEndMinutes: Math.max(
            1,
            Math.min(30, Number(result.data.before_end_minutes || 5)),
          ),
          endEnabled: result.data.end_enabled !== false,
          phoneNotificationEnabled:
            result.data.phone_notification_enabled !== false,
        };
      }
    } catch (error) {}
  }
  ...
  function check() {
    if (
      !settings.enabled ||
      !settings.phoneNotificationEnabled ||
      typeof window.getSchedule !== "function"
    )
      return;
    ...
    if (settings.endEnabled && lastCurrentKey && endSent !== lastCurrentKey) {
      ...
      showPhoneNotification(
        ...
        "school-timer-end-" + lastCurrentKey,
      );
    }
  }
