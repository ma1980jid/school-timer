(function(){
  ...
  function requireAdminCode(){
    const key = 'school_timer_admin_code_' + slug();
  ...
  function decode(text){
    ...
    try { return JSON.parse(value.slice(PREFIX.length)); } catch (error) { return null; }
  }

  function slotTexts(item){
    if (item.tickerText || item.rightText || item.leftText) return item;
    const title = clean(item.title);
    const text = clean(item.text);
    const legacy = title && text ? `${title}: ${text}` : (text || title);
    const target = clean(item.target) || 'ticker';
    return {
      ...item,
      tickerText: target === 'ticker' || target === 'all' ? legacy : '',
      rightText: target === 'right' || target === 'all' ? legacy : '',
      leftText: target === 'left' || target === 'all' ? legacy : ''
    };
  }
  ...
  function styles(){
    ...
    style.textContent = `
      ...
      .scheduled-row-head{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center}.scheduled-row-title{font-weight:900;color:#0f172a}
      .scheduled-row-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.scheduled-row label{display:block;font-size:13px;color:#64748b;font-weight:900;margin-bottom:4px;text-align:right}
      ...
      .scheduled-row textarea{min-height:58px;resize:vertical;line-height:1.55}.scheduled-row input:focus,.scheduled-row textarea:focus{border-color:#14b8a6;box-shadow:0 0 0 3px rgba(20,184,166,.12)}
      .scheduled-note{margin:0;color:#64748b;font-weight:900;line-height:1.8}.scheduled-actions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:10px;position:sticky;bottom:0;z-index:5;background:linear-gradient(180deg,rgba(255,255,255,.72),#fff 42%);padding-top:12px;padding-bottom:4px}
      .scheduled-chip{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:5px 10px;background:#ecfeff;color:#0f766e;font-size:12px;font-weight:900}.scheduled-slot{border:1px dashed #cbd5e1;border-radius:14px;padding:8px;background:white}.scheduled-slot label{color:#0f766e}
  ...
  function createRow(item = {}){
    const data = slotTexts(item);
    const start = data.start || today();
    const end = data.end || data.start || start;
    const row = document.createElement('div');
    row.className = 'scheduled-row';
    row.dataset.id = data.id || String(Date.now()) + Math.random().toString(16).slice(2);
    row.innerHTML = `
      <div class="scheduled-row-head"><span class="scheduled-row-title">إعلان بتاريخ محدد</span><button class="mini del" type="button">حذف</button></div>
      <div class="scheduled-row-grid">
        <div><label>عنوان الإعلان</label><input class="scheduledTitle" value="${escapeHtml(data.title || '')}" placeholder="مثال: اليوم الوطني"></div>
      ...
      <div class="scheduled-slot"><label>عبارة شريط الرسائل</label><textarea class="scheduledTickerText" placeholder="تظهر في الشريط السفلي المتحرك">${escapeHtml(data.tickerText || '')}</textarea></div>
      <div class="scheduled-row-grid"><div class="scheduled-slot"><label>عبارة الصندوق الأيمن</label><textarea class="scheduledRightText" placeholder="تظهر في الصندوق الأيمن">${escapeHtml(data.rightText || '')}</textarea></div><div class="scheduled-slot"><label>عبارة الصندوق الأيسر</label><textarea class="scheduledLeftText" placeholder="تظهر ثابتة في الصندوق الأيسر">${escapeHtml(data.leftText || '')}</textarea></div></div>
    `;
    row.querySelector('.scheduledAnnual').checked = !!data.annual;
    row.querySelector('.scheduledActive').checked = data.active !== false;
  ...
  function ensureDialog(){
    ...
    dialog.innerHTML = `<div><h3>الإعلانات المجدولة</h3><p class="scheduled-note">اربط الإعلان بتاريخ محدد، ويمكنك كتابة عبارة مختلفة للشريط وللصندوقين في الإعلان نفسه.</p><div id="scheduledAnnouncementsList" class="scheduled-list"></div><div class="scheduled-actions"><button class="btn light" type="button" id="addScheduledAnnouncementBtn">إضافة إعلان</button><button class="btn navy" type="button" id="saveScheduledAnnouncementsBtn">حفظ</button><button class="btn" type="button" id="closeScheduledAnnouncementsBtn">إغلاق</button></div></div>`;
  ...
  function addRow(item){
    ...
    if (list) list.appendChild(createRow(item));
  }
  ...
  async function loadAnnouncements(){
    ...
    const example = { title:'اليوم الوطني', tickerText:'كل عام وعماننا بخير', rightText:'اليوم الوطني المجيد', leftText:'دامت عمان عزًا وفخرًا', start:today(), end:today(), annual:true };
    const db = client();
    if (!db) { addRow(example); return; }
    try {
      const { data, error } = await db.from('school_messages').select('message_text,sort_order').eq('school_slug', slug()).eq('is_active', true).order('sort_order', { ascending: true });
      if (error) return;
  ...
  async function saveAnnouncements(){
    if (isSaving) return;
    const db = client();
    if (!db) return toast('لم يتم ضبط اتصال Supabase');
    const code = requireAdminCode();
    if (!code) return toast('لم يتم إدخال رمز الإدارة');
    const items = collectRows();
    if (!items.length) return toast('أضف إعلانًا واحدًا على الأقل');
    ...
    try {
      ...
      const { data: ok, error: authError } = await db.from('schools').select('school_slug').eq('school_slug', schoolSlug).eq('admin_code', code).maybeSingle();
      if (authError || !ok) { sessionStorage.removeItem('school_timer_admin_code_' + schoolSlug); return toast('رمز الإدارة غير صحيح'); }
      const { error: delError } = await db.from('school_messages').delete().eq('school_slug', schoolSlug).like('message_text', PREFIX + '%');
      if (delError) return toast('تعذر تحديث الإعلانات المجدولة');
      const rows = items.map((item, index) => ({ school_slug: schoolSlug, message_text: encode(item), is_active: true, sort_order: 8000 + index }));
      const { error: insError } = await db.from('school_messages').insert(rows);
      if (insError) return toast('تعذر حفظ الإعلانات المجدولة');
      ...
      toast('تم حفظ الإعلانات المجدولة بنجاح');
    } finally {
      isSaving = false;
  ...
  function addButton(){
    ...
    btn.type = 'button';
    btn.textContent = 'إعلانات بتاريخ محدد';
