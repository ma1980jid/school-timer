(function () {
  ...
  function db() {
    ...
    if (!window.__alertSettingsClient) {
      window.__alertSettingsClient = window.supabase.createClient(
        window.SCHOOL_TIMER_SUPABASE_URL,
        window.SCHOOL_TIMER_SUPABASE_ANON_KEY,
      );
    }
  ...
  function makeButton() {
    ...
    const anchor =
      buttons.find((button) =>
        /الإعلانات المجدولة/.test(button.textContent || ""),
      ) ||
      buttons.find((button) =>
        /إدارة الرسائل/.test(button.textContent || ""),
      ) ||
      buttons.at(-1);
  ...
  async function loadTableSettings(client) {
    try {
      const result = await client
        ...
        .select(
          "enabled,before_end_enabled,before_end_minutes,end_enabled,phone_notification_enabled,sound_enabled,sound_url,updated_at",
        )
        .eq("school_slug", slug())
        .maybeSingle();
      if (!result.error && result.data)
        return { ...DEFAULTS, ...fromTable(result.data) };
    } catch (error) {}
    return null;
  }
    ...
    try {
      ...
      if (result.data && result.data[0]) {
        return {
          ...DEFAULTS,
          ...JSON.parse(
            String(result.data[0].message_text).slice(PREFIX.length),
          ),
          soundEnabled: false,
          soundUrl: "",
        };
  ...
  async function saveTableSettings(client, settings) {
    const payload = {
      ...
      before_end_minutes: Math.max(
        1,
        Math.min(30, Number(settings.beforeEndMinutes || 5)),
      ),
  ...
  async function saveLog(client, settings) {
    try {
      await client.from("system_logs").insert({
        ...
        details:
          "تم حفظ إعدادات التنبيهات في school_alert_settings وفي __ALERT_SETTINGS__ مؤقتًا.",
      });
    } catch (error) {}
  }
  ...
  async function openDialog() {
    ...
    box.innerHTML =
      "<h2>تنبيهات الهاتف</h2><p>إعدادات مختصرة للتنبيه قبل نهاية الحصة وعند نهايتها.</p>";

    const enabled = Object.assign(document.createElement("input"), {
      type: "checkbox",
      checked: !!settings.enabled,
    });
    const beforeEndEnabled = Object.assign(document.createElement("input"), {
      type: "checkbox",
      checked: !!settings.beforeEndEnabled,
    });
    const beforeEndMinutes = Object.assign(document.createElement("input"), {
      type: "number",
      min: "1",
      max: "30",
      value: Number(settings.beforeEndMinutes || 5),
    });
    const endEnabled = Object.assign(document.createElement("input"), {
      type: "checkbox",
    ...
    box.append(
      ...
      row(
        "تنبيه قبل نهاية الحصة",
        "يظهر عداد تنازلي داخل شاشة المؤقت.",
        beforeEndEnabled,
      ),
      row(
        "عدد دقائق التنبيه قبل النهاية",
        "من 1 إلى 30 دقيقة.",
        beforeEndMinutes,
      ),
