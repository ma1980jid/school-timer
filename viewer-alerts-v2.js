(function () {
  ...
  function tone(c, f, d) {
    try {
      ...
      g.gain.exponentialRampToValueAtTime(0.26, c.currentTime + d + 0.03);
      g.gain.exponentialRampToValueAtTime(0.001, c.currentTime + d + 0.38);
      o.connect(g);
      g.connect(c.destination);
      o.start(c.currentTime + d);
      o.stop(c.currentTime + d + 0.42);
    } catch (e) {}
  }
  ...
  function permission() {
    if (
      ...
      document.getElementById("enableViewerAlerts")
    )
      return;
    style();
  ...
  function applyTableSettings(row) {
    ...
    cfg.beforeEndMinutes = Math.max(
      1,
      Math.min(30, Number(cfg.beforeEndMinutes || 5)),
    );
    permission();
    return true;
  }
  ...
  function applyLegacySettings(text) {
    try {
      cfg = Object.assign(
        cfg,
        JSON.parse(String(text || "").slice(PREFIX.length)),
      );
      cfg.beforeEndMinutes = Math.max(
        1,
        Math.min(30, Number(cfg.beforeEndMinutes || 5)),
  ...
  async function load() {
    ...
    try {
      ...
      if (legacy.data && legacy.data[0])
        applyLegacySettings(legacy.data[0].message_text);
    } catch (e) {}
  }
  ...
  function check() {
    ...
    if (cfg.endEnabled && lastKey && ended !== lastKey)
      fireEnd({ name: "السابقة" }, lastKey);
  }
