...
const P='__SCHEDULE_ROWS__:',slug=new URLSearchParams(location.search).get('school')||window.SCHOOL_TIMER_SLUG||'alsheikh-saif';let rows=[],sig='';
...
function time(id,x){let e=document.getElementById(id);if(!e)return;let v=range(x);if(e.textContent!==v)e.textContent=v;e.setAttribute('dir','ltr')}
...
function renderTable(s=schedule()){let c1=document.getElementById('scheduleCol1'),c2=document.getElementById('scheduleCol2');if(!c1||!c2||!s.list.length)return;let ns=s.list.map(x=>(s.current===x?'جارية':s.currentMinutes>=x.endMinutes?'انتهت':'قادمة')+'|'+x.name+'|'+x.start+'|'+x.end).join('||');if(ns===sig)return;sig=ns;c1.replaceChildren(...s.list.filter(x=>x.col===1).map(x=>tr(x,s.current,s.currentMinutes)));c2.replaceChildren(...s.list.filter(x=>x.col===2).map(x=>tr(x,s.current,s.currentMinutes)))}
function updateCards(s=schedule()){set('previousName',s.previous?s.previous.name:'--');time('previousTime',s.previous);set('nextName',s.next?s.next.name:'--');time('nextTime',s.next);if(s.current){set('currentName',s.current.name);time('currentTime',s.current)}else if(s.next){set('currentName','الحصة\nالخالية');time('currentTime',s.next)}else{set('currentName','انتهى\nالدوام');time('currentTime',null)}}
function updateRemaining(s=schedule()){let sec=s.time.h*3600+s.time.m*60+s.time.s;if(s.current){set('countLabel','متبقي من الحصة الحالية');set('remainingTime',dur(s.current.endMinutes*60-sec))}else if(s.next){set('countLabel','متبقي على الحصة القادمة');set('remainingTime',dur(s.next.startMinutes*60-sec))}else{set('countLabel','انتهى اليوم الدراسي');set('remainingTime','00:00')}}
