(function(){
  if (window.__viewerAutoThemeV2Loaded) return;
  window.__viewerAutoThemeV2Loaded = true;
  ...
  async function loadAutoTheme(){
    ...
    try {
      const [autoResult, eventResult] = await Promise.all([
        db.from('school_messages').select('message_text,created_at').eq('school_slug', schoolSlug).like('message_text', AUTO_PREFIX + '%').order('created_at', { ascending:false }).limit(1),
