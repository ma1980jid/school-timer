(function () {
  ...
  function remainingSeconds(schedule) {
    ...
    return Math.floor(
      schedule.current.endMinutes * 60 - (h * 3600 + m * 60 + s),
    );
  }
  ...
  async function loadSettings() {
    ...
    try {
      const table = await c
        ...
        .select(
          "enabled,before_end_enabled,before_end_minutes,end_enabled,phone_notification_enabled",
        )
        .eq("school_slug", slug)
        .maybeSingle();
      if (!table.error && table.data) {
        settings = {
          ...
          beforeEndMinutes: Math.max(
            1,
            Math.min(30, Number(table.data.before_end_minutes || 5)),
          ),
          endEnabled: table.data.end_enabled !== false,
          phoneNotificationEnabled:
            table.data.phone_notification_enabled !== false,
        };
        return;
      }
    } catch (error) {}
    ...
    try {
      ...
      if (r.data && r.data[0]) {
        settings = Object.assign(
          settings,
          JSON.parse(String(r.data[0].message_text).slice(PREFIX.length)),
        );
  ...
  function ensurePermissionButton() {
    ...
    button.style.cssText =
      "position:fixed;left:16px;bottom:16px;z-index:1000000;border:0;border-radius:14px;background:#0f766e;color:#fff;padding:11px 14px;font-family:Tahoma,Arial,sans-serif;font-weight:900;box-shadow:0 10px 24px rgba(15,23,42,.24)";
  ...
  function sendNotification(message) {
    ...
    try {
      new Notification(message, {
        ...
        badge: "icons/notification-bell.svg",
      });
    } catch (error) {}
  }
  ...
  function closeStaleScreenAlert(key) {
    try {
      if (lastSeenCurrent && key && lastSeenCurrent !== key) {
        document
          .querySelectorAll(".viewer-alert-toast")
  ...
  function check() {
    ...
    if (current && key) {
      ...
      lastSeenCurrent = key;
      const beforeSeconds =
        Math.max(1, Number(settings.beforeEndMinutes || 5)) * 60;
      if (
        settings.beforeEndEnabled &&
        remaining !== null &&
        remaining <= beforeSeconds &&
        remaining > 0 &&
        notifiedBefore !== key
      ) {
        notifiedBefore = key;
        sendNotification(
          `باقي ${Math.ceil(remaining / 60)} د من الحصة ${current.name}`,
        );
      }
      if (
        settings.endEnabled &&
    ...
    if (settings.endEnabled && lastCurrent && notifiedEnd !== lastCurrent) {
      ...
      sendNotification(
        lastCurrentName ? `انتهت الحصة ${lastCurrentName}` : "انتهت الحصة",
      );
    }
  }
