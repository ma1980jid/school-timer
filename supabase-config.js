...
window.SCHOOL_TIMER_SUPABASE_ANON_KEY = [
  "sb",
  "publishable",
  "SMJPGEWFq0B0nCyMvu9Sbg",
  "kt7N5hd5",
].join("_");
window.SCHOOL_TIMER_SLUG =
  new URLSearchParams(location.search).get("school") || "__neutral__";
...
(function ensureLocalSupabaseFallback() {
  ...
  function createClient(baseUrl, anonKey) {
    ...
    class QueryBuilder {
      ...
      like(column, value) {
        this.params.append(column, "like." + String(value));
        return this;
      }
      ilike(column, value) {
      ...
      not(column, operator, value) {
        this.params.append(
          column,
          "not." + String(operator) + "." + String(value),
        );
        return this;
      }
      match(values) {
      ...
      or(expression) {
        this.params.set("or", "(" + String(expression || "") + ")");
        return this;
      }
      ...
      async execute() {
        try {
          ...
          if (
            Number.isFinite(this.rangeFrom) &&
            Number.isFinite(this.rangeTo)
          ) {
    ...
    async function rpc(functionName, payload = {}) {
      try {
        const response = await fetch(
          root + "/rest/v1/rpc/" + encodeURIComponent(functionName),
          {
            method: "POST",
...
if (isDashboardPage) {
  ...
  loadScript("dashboard-alert-settings.js?v=alerts-dashboard-04-silent");
  ...
  window.addEventListener("load", function () {
    loadScript("viewer-alerts-v2.js?v=viewer-alerts-05-silent-bell");
  });
}
